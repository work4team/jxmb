...
class LibLogin{
	...
	public function set_password($pwd,$md5=false,$sha=false){
        if($sha) $this->password = $pwd;
        elseif($md5)$this->password = sha1($pwd);
		else $this->password = sha1(md5($pwd));
	}
	...
	private function auto_login(){
		...
		if($pass){
			$pass = explode("\n",$pass);
			$this->set_password(trim($pass[0]),true,true);
